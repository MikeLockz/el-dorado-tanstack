...
- **Outbound routing:** Every container that needs to emit telemetry must resolve and reach `OBSERVABILITY_LXC_IP`. Ensure the host firewall allows outbound `tcp/3100`, `tcp/4317`, and `tcp/9187`.
...
- **Host port exposure:** The optional `postgres-exporter` maps port `9187` to the host so that Prometheus on the LXC can scrape it over the LXC-to-host bridge.
...
- The compose extension, promtail config, and backend/nginx OTLP exporters all reference this variable. `docker compose config` will fail fast if it is missing thanks to the `${VAR:?error}` syntax in `app-docker-compose-extension.yml`.
...
- On the observability LXC, define `APP_STACK_HOST_IP` (the routable address of the Docker host that runs this stack). Prometheus uses it inside `config/prometheus-scrape-app.yml` to reach `postgres-exporter` on port `9187`.
...
1. Set `OBSERVABILITY_LXC_IP` locally and run `docker compose -f docker-compose.yml -f app-docker-compose-extension.yml up -d promtail-sidecar`.
