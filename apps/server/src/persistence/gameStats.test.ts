...
describe('computeGameStats', () => {
  ...
  it('counts misplays from event log', () => {
    const mockRoom = createMockRoom({
      eventLog: [
        { type: 'INVALID_ACTION', payload: { playerId: 'p1', action: 'PLAY', reason: 'bad' }, eventIndex: 0, createdAt: new Date() },
        { type: 'INVALID_ACTION', payload: { playerId: 'p1', action: 'BID', reason: 'bad' }, eventIndex: 1, createdAt: new Date() },
        { type: 'GAME_STARTED', payload: {}, eventIndex: 2, createdAt: new Date() },
      ],
    });
